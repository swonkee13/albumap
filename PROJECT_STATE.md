...
_Last updated: 2026-07-31 (v2 → v2.8 change sessions — see the change logs below)_

**v2.8 (latest — hero redesign + completion gauges):** The album header now leads with the **album cover** (sized to the text-column height, JS-fit) with the **band logo badged on its corner** (Option B). The single parts-done ring was replaced by a **configurable gauge panel** (an "instrument panel" of dials) — choose any of **Recording / Artwork / Photos / Logos / Merch / Overall** via a gear-button picker (selection saved per album in `localStorage:'albumap:hero'`, default `['recording']`). Recording = grid parts %; Artwork/Photos/Logos/Merch = **finalized ÷ total items**; Overall = average of the others. Items get a **"finalized" flag** — a green ✓ toggle in the artwork/photo/logo zoom modal (+ tile badge/outline) and on each merch item (+ collapsed badge). New columns `artwork_pieces.finalized`, `merch_items.finalized` (persisted via `/api/artwork` update + `/api/merch-item` update). **Migration required — two `finalized` columns (SQL below).**
